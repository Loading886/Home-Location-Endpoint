...
6. 拦截器终止该内层 TLS，以 HTTP/1.1 向真实 Apple 源站重新发起请求，随后平移 WLOC 或
   WifiTile 响应中的坐标；超过 45 m 的整片热点几何会等比例压缩到目标周围，小型几何保持原尺寸。
...
- 同一批次保留所有 AP/蜂窝点之间的距离比例和方向；最大偏移超过 45 m 时统一缩小，原本较小的
  几何不会被放大；
...
1. WLOC 只有在整个可识别批次均为 sentinel 且少于 32 条时，才从最近 30 分钟同一进程看到的
   真实 Wi-Fi 身份补足至最多 32 条，再生成以目标为中心、半径不超过 45 m 的稳定坐标簇。
2. 正常 WifiTile 200 先完成平移和限幅，再把手机近期 WLOC 请求中出现、但原 tile 没有有效坐标的
   BSSID 去重后追加为独立 protobuf region；WLOC 响应侧学到的身份不会进入正常 200 补充路径。
3. 精确 WifiTile 404 先复用 24 小时内缓存的完整 Apple tile，并平移、限幅其几何关系。
4. 没有缓存时请求一个已知有效的 Apple 公共 seed tile，并平移、限幅到目标；仍失败时，才用近期真实
   Wi-Fi 身份构造最小 tile。
5. 没有任何可信身份或模板时保留 Apple 原始 404；可选回退失败不会扩大为定位服务整体故障。

综合恢复缓存和请求侧补充缓存分别最多保留 256 个 Wi-Fi 身份、仅在内存中存在，服务重启即清空；
完整模板也只保留一份。运行日志只记录分支、数量、补充数量和字节数，不记录 BSSID、坐标、
tile key、请求体或响应体。

所有内存缓存都与活动目标无关：身份缓存只保存标识，完整 tile 每次使用时都会重新变换到当前目标。
切换地点不会主动清空缓存。坐标改写会移除已经失效的 `Content-Encoding`，但保留 Apple 原始
`Cache-Control`、`Age`、`ETag` 和 `Last-Modified` 契约；强制 `no-store` 会破坏 iOS 的正常收敛。
由于 iOS 仍可能使用设备端十余分钟的定位缓存，这只改善下一次请求到达后的收敛，不承诺立即切换。
