...
2. 精确 WifiTile 404 先复用 24 小时内缓存的完整 Apple tile，并平移、限幅其几何关系。
3. 没有缓存时请求一个已知有效的 Apple 公共 seed tile，并平移、限幅到目标；仍失败时，才用近期真实
   Wi-Fi 身份构造最小 tile。
...
两个内存缓存都与活动目标无关：身份缓存只保存标识，完整 tile 每次使用时都会重新变换到当前目标。
