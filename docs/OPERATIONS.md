...
```bash
curl -fsSL https://raw.githubusercontent.com/Loading886/Home-Location-Endpoint/v0.1.10/install.sh \
  | sudo env HLE_VERSION=v0.1.10 bash
```
...
`hle verify` 验证服务端状态；完整模式还应至少运行一次真实 VLESS + REALITY 客户端测试。直接在
已安装节点上验证回环和出口：

```bash
sudo ./tests/e2e_installed_endpoint.sh --expected-egress SERVER_EGRESS_IP
```
...
两套脚本都会启动临时 SOCKS 客户端、核对实际出口并请求 HTTP 204。URI 文件和环境变量属于
