...
```bash
curl -fsSL https://raw.githubusercontent.com/Loading886/Home-Location-Endpoint/v0.2.0/install.sh \
  | sudo env HLE_VERSION=v0.2.0 bash
```
...
Bot API 心跳。服务 active 但心跳 FAIL 时，优先检查 Token 是否被撤销、服务器到 Telegram 的
HTTPS 网络，以及是否有另一程序在使用同一 Token 轮询 `getUpdates`。
