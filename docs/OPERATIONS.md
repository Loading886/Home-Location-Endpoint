...
iOS 可能在设备端保留十余分钟的热点结果或 `notindb` 负缓存，服务端无法主动清除。先确认日志持续
出现成功的 `TRANSLATE` 或 `WIFITILE_TRANSLATE`；后者的 `injected=N` 只表示补入数量，不包含身份。
若服务端一直正常但手机仍无定位，可正常重启 iPhone 一次，让系统自行重建定位缓存。不要用 Xcode
`simulate-location set`/`clear` 反复刺激缓存，也不要通过重启拦截器、清空服务端内存缓存或强制
`Cache-Control: no-store` 尝试催促切换；这些做法可能延长无定位状态。
